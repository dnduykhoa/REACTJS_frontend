...
import { ShoppingCart, Trash2, Plus, Minus, Package, ArrowRight, AlertTriangle, XCircle } from 'lucide-react';
...
export default function CartPage() {
  ...
  const hasOutOfStock = cart?.items.some((item) => !item.inStock) ?? false;
            ...
            return (
              <div
                ...
                className={`flex gap-4 bg-white rounded-xl border p-4 shadow-sm transition-opacity ${
                  ...
                  !item.inStock ? 'border-rose-200 bg-rose-50/30' : 'border-slate-100'
                }`}
              >
                {/* Image */}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-between mt-3 flex-wrap gap-2">
                    ...
                    <div className={`flex items-center border rounded-lg overflow-hidden ${
                      !item.inStock ? 'border-rose-200 opacity-50' : 'border-slate-200'
                    }`}>
                      <button
                    ...
                    <div className="flex items-center gap-3">
                      <p className={`text-base font-bold ${
                        !item.inStock ? 'text-slate-400 line-through' : 'text-indigo-600'
                      }`}>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-5 sticky top-24">
              ...
              {cart.items.map((item) => (
                <div key={item.id} className="flex justify-between">
                  <span className={`truncate max-w-[60%] ${
                    !item.inStock ? 'text-rose-400 line-through' : 'text-slate-500'
            ...
            <div className="border-t border-slate-100 pt-3 mb-4">
              ...
              {hasOutOfStock && (
                <p className="text-xs text-slate-400 mt-1">(Không bao gồm sản phẩm hết hàng)</p>
              )}
            </div>

            {hasOutOfStock && (
              <div className="flex items-start gap-2 bg-rose-50 border border-rose-200 text-rose-700 text-xs px-3 py-2.5 rounded-xl mb-3">
                <AlertTriangle className="w-3.5 h-3.5 shrink-0 mt-0.5" />
                <span>Giỏ hàng có sản phẩm hết hàng. Vui lòng xóa trước khi thanh toán.</span>
              </div>
            )}
